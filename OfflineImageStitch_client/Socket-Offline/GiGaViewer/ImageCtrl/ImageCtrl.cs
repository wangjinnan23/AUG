...
namespace ImageCtrl
{
    ...
    {
        int port =2000;  //2000  6666
        public Point m_RightDownOnImgRated = new Point();//add

        string host = "127.0.0.1";    //IP地址和端口号 
      ...
      //      Switch of Online and Offline
        ...
        public Bitmap GetImage(int frameNum, int ScaleIndex, Rectangle m_showImgRect)
        {
            ...
            m_RightDownOnImgRated.Y = m_showImgRect.Y + m_showImgRect.Height * ScaleIndex;
              ...
              int[] temp = new int[8] { 2, 1, 682, 260, 3100, 7146, 8000, 9000 };/*{ 0, 1, m_showImgRect.Width, m_showImgRect.Height, m_showImgRect.X, m_RightDownOnImgRated.X, m_showImgRect.Y, m_RightDownOnImgRated.Y };*/
...
//           int a = int.Parse(Console.ReadLine());
              int[] temp20 = new int[8] { 2, 1, 100, 100, 100, 100, 1000, 10000 };
              byte[] message20 = new byte[32];
              for (int i = 0; i < 8; i++)
              {
                  (System.BitConverter.GetBytes(temp20[i])).CopyTo(message20, i * 4);

              }   
              m_socket.Send(message20);
