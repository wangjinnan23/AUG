...
#ifdef _WINDOWS
const string ImageStitch::FILENAME_PARAMS = "E:\\Library\\data\\ImageStitch\\OfflineImageStitch\\Verification\\config\\ImgStitch.config";
#else
...
//	for(int ScaleIdx = 0; ScaleIdx < m_nScaleCnt; ScaleIdx++) {
	for(int ScaleIdx = 0; ScaleIdx < 1; ScaleIdx++) {
...
printf("1 Process %d open MapTable %s . \n",m_nPidx,filename);
	...
	printf("Process %d open %d files. %s . \n",m_nPidx,fileCnt,filename);
...
//	File_write.open("E:/Workspace/VS2010/Aug/OfflineImageStitch/Block_Cam_points.txt",ios::out|ios::binary);
 ...
 File_write<<"The total points number is "<<cnt<<"\r\n";
 int sum_check=0;
//
